    ...
    while (true) {
        main_task_timer.reset();
            ...
            if (sensorBar.isAnyLedActive()) {
                angle = sensorBar.getAvgAngleRad() * 1.0f;
            }
        ...
        if (do_execute_main_task) {
            ...
            switch (robot_state) {
                case RobotState::FOLLOW:
                        ...
                        robot_coord = {speed * wheel_vel_max * r1_wheel, // half of the max. forward velocity
                ...
                case RobotState::WALL_TURN:
                    if ((fabs(motor_M1.getRotationTarget() - motor_M1.getRotation()) < angle_threshold) &&
                        ...
                        garbage = motor_M1.getRotation() + 0.4f;
                        garbage2 = motor_M2.getRotation() + 0.4f;
                        motor_M1.setRotation(garbage); 
                        motor_M2.setRotation(garbage2 - 0.8f);
                ...
                case RobotState::WALL_FORWARD:
                    if ((fabs(motor_M1.getRotationTarget() - motor_M1.getRotation()) < angle_threshold) &&
                         (fabs(motor_M2.getRotationTarget() - motor_M2.getRotation()) < angle_threshold)) {
                        garbage = motor_M1.getRotation() + 2.9f;
                        garbage2 = motor_M2.getRotation() + 2.9f;
                        motor_M1.setRotation(garbage); 
                ...
                case RobotState::WALL_TURN_90:
                    if ((fabs(motor_M1.getRotationTarget() - motor_M1.getRotation()) < angle_threshold) &&
                         (fabs(motor_M2.getRotationTarget() - motor_M2.getRotation()) < angle_threshold)) {
                        garbage = motor_M1.getRotation() + 0.8f;
                        garbage2 = motor_M2.getRotation() + 0.8f;
                        motor_M1.setRotation(garbage - 1.6f); 
                        motor_M2.setRotation(garbage2);
                ...
                case RobotState::WALL_FORWARD_2:
                    if ((fabs(motor_M1.getRotationTarget() - motor_M1.getRotation()) < angle_threshold) &&
                         (fabs(motor_M2.getRotationTarget() - motor_M2.getRotation()) < angle_threshold)) {
                        garbage = motor_M1.getRotation() + 2.4f;
                        garbage2 = motor_M2.getRotation() + 2.4f;
                        motor_M1.setRotation(garbage); 
                        motor_M2.setRotation(garbage2);
                ...
                case RobotState::BACK:
                    garbage = motor_M1.getRotation() + 0.1f;
                    garbage2 = motor_M2.getRotation() + 0.1f;
                    motor_M1.setRotation(garbage); 
                    motor_M2.setRotation(garbage2);
                ...
                case RobotState::ROTATE:
                    ...
                    garbage = motor_M1.getRotation() - 1.5f;
                    garbage2 = motor_M2.getRotation() + 1.5f;
                    motor_M1.setRotation(garbage); 
                    motor_M2.setRotation(garbage2);
        ...
        printf(" angle Output: %.2f", angle);

        printf("controller val: %f", Kp * angle + Kp_nl * copysignf(angle * angle, angle));


        //printf(" wheel speed: %.2f", ;
        //printf(" wheel speed: %.2f", angle);

        //printf(" kp_nl Output: %f", m_Kp_nl);

        // printf(" wheel speed M2: %.2f", motor_M2.getVelocity());
        // printf(" wheel speed M1: %.2f", motor_M1.getVelocity());
